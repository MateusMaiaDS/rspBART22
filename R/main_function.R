...
rspBART <- function(x_train,
  ...
  if(plot_preview){
    ...
    for(visu_basis in 1:10){
      plot(x  = 0, xlim = c(0,1), ylim = c(-1,1),
           type= 'n', ylab= paste0("B.",visu_basis), main = paste0("B(",visu_basis,")"))
      for(basis_col in 1:NCOL(B_train_obj[[visu_basis]])){
        points(x_test[,visu_basis], B_test_obj[[visu_basis]][,basis_col],
             col = basis_col, pch = 20, xlab = paste0("x.",visu_basis))
      }
    }
  ...
  P_inv <- chol2inv(chol(as.matrix(Matrix::nearPD(P_train_main)$mat)))
  # P_inv_two <- chol2inv(chol(P_train_main_two))
  P_inv_interaction <- chol2inv(chol(as.matrix(Matrix::nearPD(P_train_interaction)$mat)))
  ...
  for(i in 1:n_mcmc){
    ...
    for(t in 1:data$n_tree){
      ...
      forest[[t]] <- updateGamma(tree = forest[[t]],
                                 curr_part_res = partial_residuals,data = data)
