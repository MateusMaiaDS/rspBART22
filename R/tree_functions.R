...
nodeLogLike <- function(curr_part_res,
  ...
  cov_aux <- (diag(nrow = n_leaf) + cov_aux + data$tau_gamma^(-1))/data$tau
...
updateBetas <- function(tree,
  ...
  intercept_term_train <- numeric(nrow(data$x_train))
  intercept_term_test <- numeric(nrow(data$x_test))
  ...
  for(i in 1:length(t_nodes_names)){
    ...
    for(jj in 1:length(node_index_var)){
      ...
      res_leaf <- matrix(curr_part_res[cu_t$train_index], ncol=1) - (pred_minus_jj + cu_t$gamma) # Also removing the intercept
      ...
      intercept_term_train[cu_t$train_index] <- cu_t$gamma
      intercept_term_test[cu_t$test_index] <- cu_t$gamma
  ...
  tree_hat_train <- rowSums(y_hat_train) + intercept_term_train
  tree_hat_test <- rowSums(y_hat_test) + intercept_term_test

  # Returning the tree
  return(list(tree = tree,
              y_hat_train = y_hat_train,
              intercept_term_train = intercept_term_train,
              intercept_term_test = intercept_term_test,
...
updateGamma <- function(tree,
  ...
  for(i in 1:length(t_nodes_names)){
    ...
    # Getting the splines prediction
    pred_splines <- numeric(n_leaf)
...
update_tau <- function(y_train_hat,
  ...
  for(tree in 1:length(forest)){
    ...
    for(curr_node in terminal_nodes_names) {
      ...
      intercept_sum <- intercept_sum + data$tau_gamma*forest[[tree]][[curr_node]]$gamma^2
    }
  }

  tau_sample <- stats::rgamma(n = 1,shape = 0.5*n_ + 0.5*sum(k_vec) + data$a_tau,
                              rate = 0.5*crossprod((data$y_train-y_train_hat))+ 0.5*sum(j_vec) + 0.5*intercept_sum + data$d_tau)
